...
def plot_2d_summary(list_of_dfs: list, subject_id: str, task_name: str, output_dir: str):
    ...
    # Calculation of the ellipse based on total variability
    ...
    #ax.plot(TRIANGLE_POINTS[:, 0], TRIANGLE_POINTS[:, 1], color='gray', linestyle='--', linewidth=1.5, label='Sensor Perimeter')
    ax.scatter(combined_cop_data[:, 0], combined_cop_data[:, 1], c='lightblue', s=5, alpha=0.4, label=f'CoP Variability (N={len(list_of_dfs)})')
    ...
    ellipse = patches.Ellipse(xy=mean_cop_total, width=width, height=height, angle=angle,
                              fill=False, edgecolor='red', linewidth=2.5, label='Variability Ellipse (95%)')
    ...
    output_path = os.path.join(output_dir, f"{subject_id}_{task_name}_2d_summary_ellipse.png")
